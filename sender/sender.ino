...
int SpeedPin = 32;
int Switch=18;
int s=0; // speed pin from other joystick


// REPLACE WITH THE MAC Address of your receiver 
uint8_t broadcastAddress[] = {0x08, 0xB6, 0x1F, 0x3B, 0x8B, 0x04};
...
int cmd()
{
  int valX = digi(myData.x);    // wirte the value of X
  int valY = digi(myData.y);
 ...
 if(s==LOW)
  ...
  if ((valX==0)and (valY==1)){
      return 4;}
  if(((valX==2)and(valY==2))or((valX==2)and(valY==0))or((valX==0)and(valY==2))or((valX==0)and (valY==0)))
 { return 0;
    }}
    else
    {if ((valX==1)and (valY==1)){  /// valX = 1 and valY = 1 means joystick is in it's original position. Bot should not move
...
int digi(int analval){
    if ( analval < 1500)  return 0;
    if((analval  > 1500)and (analval < 2300))  return 1;
